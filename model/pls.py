import pandas as pd
import numpy as np
from src.pls_pure import pls_pure
...
class PLS(object):
    ...
    def pls(self):
        if self.is_var_exp:
            self.b, self.var_exp_x, self.var_exp_y = pls_pure(self.xtx, self.xty,
                                                              self.x_mean, self.y_mean,
                                                              self.n, self.p,
                                                              self.is_scale,
                                                              self.is_var_exp, self.yty)
        else:
            self.b = pls_pure(self.xtx, self.xty,self.x_mean, self.y_mean,
    ...
    def cv(self, k=10):
        ...
        def cvk(index):
            ...
            tb = pls_pure(txx_, txy_, tx_mean_, ty_mean_, tn_, tp, self.is_scale)
